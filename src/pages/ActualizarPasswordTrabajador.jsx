...
/**Pantalla para que los trabajadores (usuarios de la app móvil) puedan actualizar su contraseña*/
const ActualizarPasswordTrabajador = () => {
    ...
    useEffect(() => {
        async function comprobarToken() {
            try {
                const response = await axios.get(API_URL+'/auth/recuperar-cuenta/'+token)
            } catch (e) {
    ...
    async function handleSubmit(ev) {
      ...
      try {
        const response = await axios.post(API_URL+'/auth/actualizar-password/'+token, {password})

        Swal.fire('Actualizada', `${response.data.msg}, regrese a la aplicación`, 'success')
        navigate('/', {replace: true})
      } catch (e) {
...
export default ActualizarPasswordTrabajador